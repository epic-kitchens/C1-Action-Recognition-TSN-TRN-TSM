...
class NL3DWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        nt, c, h, w = x.size()
        x = x.view(nt // self.n_segment, self.n_segment, c, h, w).transpose(
            1, 2
        )  # n, c, t, h, w
        x = self.nl(x)
...
if __name__ == "__main__":
    ...
    img = torch.zeros(2, 3, 20, requires_grad=True)
    net = NONLocalBlock1D(3, sub_sample=sub_sample, bn_layer=bn_layer)
    out = net(img)
    print(out.size())

    img = torch.zeros(2, 3, 20, 20, requires_grad=True)
    net = NONLocalBlock2D(3, sub_sample=sub_sample, bn_layer=bn_layer)
    out = net(img)
    print(out.size())

    img = torch.randn((2, 3, 10, 20, 20), requires_grad=True)
